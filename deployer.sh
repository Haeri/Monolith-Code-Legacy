...
curl -F "key=$1" -F "version=$version" -F "build=$build" -F "beta=$beta" -F "need_fresh=$fresh" -F "release_notes=$text" -F "file=@package.zip" http://monolith-code.net.tiberius.sui-inter.net/release/addRelease