...
public class CodeBuilder extends Thread {
	...
	public void begin(BuildMode mode) {
		...
		if(cce.useNativeConsole){
			if(mode == BuildMode.BUILD_N_RUN){
				...
				freeCommand = NativeConsole.createNativeRunCommand(fullname, path, runCommand); 	 			
			}
	...
	public void run() {
		...
		if (mode == BuildMode.BUILD || mode == BuildMode.BUILD_N_RUN) {
			...
			try {
				errBuild = _exec(buildCommand, path);
			} catch (Exception e) {
		...
		if(mode == BuildMode.FREE_BUILD){
			...
			if(GlobalVariables.debug) con.println(freeCommand);
				
			try {
				errFree = _exec(freeCommand, path);
			} catch (Exception e) {
				con.println(e.getMessage(), Console.err);
				if(GlobalVariables.debug) e.printStackTrace();
				errFree = true;
	...
	private boolean _exec(String command, String path) throws Exception{			
